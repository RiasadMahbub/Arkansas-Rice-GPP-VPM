...
agg_tbl_GPP <- GPPyieldmerge %>% dplyr::select(meancumgpp8dayagg, Year) ##GPPyieldmerge is from VPM_Interannual script
agg_tbl_GPP <- left_join(agg_tbl, agg_tbl_GPP, by = "Year")
# Calculate Rate_percent using mean_yield from the previous row
agg_tbl_GPP <- agg_tbl_GPP %>%
  dplyr::mutate(Rate_percent = (Diff_growth / dplyr::lag(mean_yield)) / Diff_year * 100)
agg_tbl_GPP <- agg_tbl_GPP %>%
  dplyr::mutate(GPP_Rate_percent = (c(NA, diff(meancumgpp8dayagg)) / dplyr::lag(meancumgpp8dayagg)) / Diff_year * 100)

# Filter years where Rate_percent is positive and GPP_Rate_percent is negative
pos_rate_neg_gpp <- agg_tbl_GPP %>%
  filter(Rate_percent > 0 & GPP_Rate_percent < 0)

# Filter years where GPP_Rate_percent is positive and Rate_percent is negative
neg_rate_pos_gpp <- agg_tbl_GPP %>%
  filter(Rate_percent < 0 & GPP_Rate_percent > 0)
# Print the results
print("Years where Rate_percent is positive and GPP_Rate_percent is negative:")
print(pos_rate_neg_gpp)
print("Years where GPP_Rate_percent is positive and Rate_percent is negative:")
print(neg_rate_pos_gpp)



write.csv(agg_tbl_GPP, 
...
merged_df <- merge(agg_tbl_GPP, allrasterscumulativestack_meansddf, by = "Year")
...
gppcountyyieldplot <- ggplot(data = gppricecountyarea, aes(x = gpp_sum_8day, y = yieldgm, col = Year, size = Areaplantedha)) +
  ...
  ggplot2::annotate("text", x = 1750, y = 500, label = "g~rice~m^-2~season^-1",size = 8, parse = TRUE)+
...
result_table <- result_table %>%
  mutate(  # Round R2 to 2 decimal places
    RMSE = round(RMSE, 1)  # Round RMSE to 1 decimal place
  )
...
write.csv(result_table, "result_table_rounded.csv", row.names = FALSE)
