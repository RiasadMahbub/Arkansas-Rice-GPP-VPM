...
ggsave(file = "C:/Users/rbmahbub/Documents/RProjects/VPM_Spatial/Figure/LAIresidualNoarrow.png",
...
ggplot(sitesatellitemergedDATALAI, aes(x = avg_Lai, y = residualsitevpm, color = DAP)) +
  ...
  labs(
    x = expression("Average LAI (m"^2~"m"^-2*") 8-day mean"), 
    y = expression("Residual VPM (" * g~C~m^-2~day^-1 * ") 8-day mean"), 
    color = "DAP"
  ) + 
  theme_minimal() + 
  ...
  facet_wrap(~ site.x) + # Facet by site.x column
  theme(
    axis.line = element_line(color = "black", size = 0.8) # Axis lines
  ) +
  
  # Regression line for DAP 0-80 (blue)
  geom_smooth(data = dplyr::filter(sitesatellitemergedDATALAI, DAP >= 0 & DAP <= 80), 
              method = "lm", se = FALSE, color = "blue", linewidth = 1) +
  
  
  # Regression line for DAP 80-160 (red)
  geom_smooth(data = dplyr::filter(sitesatellitemergedDATALAI, DAP > 80 & DAP <= 160), 
              method = "lm", se = FALSE, color = "red", linewidth = 1) +
  
  # Equation & R² for DAP 0-80 (blue) in top-right
  stat_poly_eq(data = dplyr::filter(sitesatellitemergedDATALAI, DAP >= 0 & DAP <= 80),
               aes(label = paste( ..rr.label.., sep = "*`,`~")), 
               formula = y ~ x, parse = TRUE, color = "blue",
               label.x.npc = "right", label.y.npc = 0.15, size = 4) + 
  
  # Equation & R² for DAP 80-160 (red) in top-right (Adjusted label.y.npc)
  stat_poly_eq(data = dplyr::filter(sitesatellitemergedDATALAI, DAP > 80 & DAP <= 160),
               aes(label = paste( ..rr.label.., sep = "*`,`~")), 
               formula = y ~ x, parse = TRUE, color = "red",
               label.x.npc = "right", label.y.npc = "bottom", size = 4) # Adjusted label.y.npc from 0.85 to 0.75


ggsave(file = "C:/Users/rbmahbub/Documents/RProjects/VPM_Spatial/Figure/LAIresidual.png",
       width = 10, height = 7, units = "in", dpi = 300)


# Function to calculate R² for given data and DAP range
calc_r2 <- function(data, dap_min, dap_max) {
  data_filtered <- data %>%
    filter(DAP >= dap_min & DAP <= dap_max)
  
  # Check how many data points are in the filtered range
  if (nrow(data_filtered) < 2) {
    warning(paste("Not enough data for DAP", dap_min, "to", dap_max))
    return(NA)  # Return NA if there are fewer than 2 data points
  }
  
  # Fit the linear model
  lm_fit <- lm(residualsitevpm ~ avg_Lai, data = data_filtered)
...
# Calculate R² for each site.x for DAP 0-80 and DAP 80-160
r2_table <- sitesatellitemergedDATALAI %>%
  group_by(site.x) %>%
  summarise(
    r2_0_80 = calc_r2(., 0, 80),  # R² for DAP 0-80
    r2_80_160 = calc_r2(., 80, 160)  # R² for DAP 80-160
  )
...
# Animated LAI vs Residual VPM plot
# Create segment data
segment_data <- sitesatellitemergedDATALAI %>%
  arrange(site.x, DAP) %>%
  group_by(site.x) %>%
  mutate(
    xend = lead(avg_Lai),
    yend = lead(residualsitevpm)
  ) %>%
  filter(!is.na(xend) & !is.na(yend))

# Plot with arrows
plot_arrows <- ggplot(segment_data, aes(x = avg_Lai, y = residualsitevpm)) +
  geom_segment(aes(xend = xend, yend = yend, color = DAP),
               arrow = arrow(length = unit(0.08, "npc"), type = "closed"),
               size = 1) +
  geom_point(aes(color = DAP), size = 2) +
  labs(
    x = expression("Average LAI (" * m^2/m^2 * ") 8-day mean"), 
    y = expression("Residual VPM (" * g~C~m^-2~day^-1 * ") 8-day mean"), 
    color = "DAP"
...
print(plot_arrows)
ggsave(file = "C:/Users/rbmahbub/Documents/RProjects/VPM_Spatial/Figure/LAIresidualarrow.png",
       width = 10, height = 7, units = "in", dpi = 300)
