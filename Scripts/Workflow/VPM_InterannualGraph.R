...
# Fit a linear model
lm_model <- lm(yield ~ meancumgpp8dayagg, data = GPPyieldmerge)
# Predict yield using the linear model
GPPyieldmerge$yield_pred <- predict(lm_model, newdata = GPPyieldmerge)
# Calculate RMSE, MAE, and Bias
rmse_value <- Metrics::rmse(GPPyieldmerge$yield, GPPyieldmerge$yield_pred)
mae_value <- Metrics::mae(GPPyieldmerge$yield, GPPyieldmerge$yield_pred)
bias_value <- mean(GPPyieldmerge$yield_pred - GPPyieldmerge$yield)
...
######################
######HARVESTINDEX###
GPPyieldmerge$HI<-(GPPyieldmerge$yield)/(GPPyieldmerge$meancumgpp8dayagg)
plot(GPPyieldmerge$Year, GPPyieldmerge$HI)

ggscatter(GPPyieldmerge, x = "Year", y = "HI", add = "reg.line", size = 7) +
  geom_point(size = 7) +
  geom_smooth(method = "lm", se = FALSE, color = "blue") +
  stat_cor(aes(label = paste(..rr.label.., ..p.label.., sep = "~`,`~")), 
           label.x = 2013.8, label.y = 0.5, size = 8) +
  stat_regline_equation(label.x = 2013.8, label.y = 0.48, size = 8) +
  #scale_x_continuous(breaks = seq(2008, 2020, by = 2)) +
  #scale_y_continuous(limits = c(0, 900), breaks = seq(0, 900, by = 100)) + 
  #geom_text(x = 2016, y = 300, label = paste("Mann-Kendall p-value =", round(mktestpvalueyield, 4)), size = 8) +
  #geom_text(x = 2015.1, y = 200, label = paste("Sens Slope =", round(senslopeyield, 2)), size = 8) +
  ...
  font("xlab", size = 24) +
  font("ylab", size = 24) +
  ylab(expression(paste("Harvest Index")))
