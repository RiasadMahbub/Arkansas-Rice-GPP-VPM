...
validation_df <- sitesatellitemergedDATA %>% 
  dplyr::filter(siteyear.x %in% validation_sites)
...
#   unique_siteyear<-unique(training_testing_df$siteyear.y)
#   num_sites_traintest <- length(unique_siteyear)
#   shuffled_siteyear <- sample(unique_siteyear)
#   
#   # Select sites for training
#   train_sites <- shuffled_siteyear[1:num_sites_traintest]
#   # Select sites for testing
#   test_sites <- shuffled_siteyear[(num_train + 1):num_sites_traintest]
#   
...
# Number of sites for training and testing
num_train <- 11
num_test <- 2

# Total number of unique siteyears
unique_siteyear <- unique(training_testing_df$siteyear.y)
num_sites <- length(unique_siteyear)
...
# Loop through different runs
for (run in 1:78) {
  # Randomly shuffle the unique siteyear values for each run
  shuffled_siteyear <- sample(unique_siteyear)
  
  # Select sites for training and testing
  train_sites <- shuffled_siteyear[1:num_train]
  test_sites <- shuffled_siteyear[(num_train + 1):(num_train + num_test)]
  
  # Print the siteyear values for training and testing sets
  cat("Run:", run, "\n")
  cat("Training sites:", train_sites, "\n")
  cat("Testing sites:", test_sites, "\n\n")
  
  # Filter the data for training and testing
  training_data <- training_testing_df[training_testing_df$siteyear.y %in% train_sites, ]
  testing_data <- training_testing_df[training_testing_df$siteyear.y %in% test_sites, ]
  
  # Calculate evaluation metrics for training data
  training_metrics <- calculate_metrics(training_data)
  cat("Evaluation Metrics for Training Data:\n")
  print(training_metrics)
  
  # Calculate evaluation metrics for testing data
  testing_metrics <- calculate_metrics(testing_data)
  cat("Evaluation Metrics for Testing Data:\n")
  print(testing_metrics)
  
  # Append information to the data frames
  training_df <- rbind(training_df, data.frame(
    Run = run,
    Training_Sites = paste(train_sites, collapse = ", "),
    LUEmax = training_metrics$LUEmax,
    Topt = training_metrics$Topt,
    RMSE_Training = training_metrics$RMSE,
    MAE_Training = training_metrics$MAE,
    Bias_Training = training_metrics$Bias,
    R_squared_Training = training_metrics$R_squared,
    combined_score_Training = training_metrics$combined_score
  ))
  
  testing_df <- rbind(testing_df, data.frame(
    Run = run,
    Testing_Sites = paste(test_sites, collapse = ", "),
    LUEmax = testing_metrics$LUEmax,
    Topt = testing_metrics$Topt,
    RMSE_Testing = testing_metrics$RMSE,
    MAE_Testing = testing_metrics$MAE,
    Bias_Testing = testing_metrics$Bias,
    R_squared_Testing = testing_metrics$R_squared,
    combined_score_Testing = testing_metrics$combined_score
  ))
}
...
cat(sprintf(
  "The mean of the training and testing sets of the 78-ensemble run of Topt were %.2f ± %.2f ºC and %.2f ± %.2f ºC respectively. The mean RMSE, MAE, bias and R2 in the training sets were %.2f g C m-2 day-1, %.2f g C m-2 day-1, %.3f g C m-2 day-1, and %.2f respectively. The mean RMSE, MAE, bias and R2 in the testing sets were slightly improved with values of %.2f g C m-2 day-1, %.2f g C m-2 day-1, %.3f g C m-2 day-1, and %.2f respectively.\n",
  mean_training_df_rounded["Topt"], sd_training_df_rounded["Topt"],  # Training Topt
  mean_testing_df_rounded["Topt"], sd_testing_df_rounded["Topt"],    # Testing Topt
  mean_training_df_rounded["RMSE_Training"],                        # Training RMSE
  mean_training_df_rounded["MAE_Training"],                         # Training MAE
  mean_training_df_rounded["Bias_Training"],                        # Training Bias
  mean_training_df_rounded["R_squared_Training"],                   # Training R2
  mean_testing_df_rounded["RMSE_Testing"],                          # Testing RMSE
  mean_testing_df_rounded["MAE_Testing"],                           # Testing MAE
  mean_testing_df_rounded["Bias_Testing"],                          # Testing Bias
  mean_testing_df_rounded["R_squared_Testing"]                      # Testing R2
))
######################################################
...
plot1 <- ggplot(evaluation_metrics, aes(x = LUEmax, y = RMSE, color = factor(Topt))) +
  ...
  labs(x = expression(paste(epsilon[0], " (mol ", CO[2], " ", mol^-1, " PPFD)")),
       y = expression(atop("RMSE of VPM predicted GPP", " (g " * C * " m"^{-2} * " day"^{-1} * ") averaged over 8 days")),
...
plot2 <- ggplot(evaluation_metrics, aes(x = LUEmax, y = MAE, color = factor(Topt))) +
  ...
  geom_line() +
  labs(x = expression(paste(epsilon[0], " (mol ", CO[2], " ", mol^-1, " PPFD)")), 
       y = expression(atop("MAE of VPM predicted GPP ", "(g " * C * " m"^{-2} * " day"^{-1} * ") averaged over 8 days")),
       color = expression("Topt (°C)")) +
  theme_minimal() +
...
plot4 <- ggplot(evaluation_metrics, aes(x = LUEmax, y = R_squared, color = factor(Topt))) +
  ...
  geom_line() +
  labs(x = expression(paste(epsilon[0], " (mol ", CO[2], " ", mol^-1, " PPFD)")),
       y = expression(atop("R"^2 ~ " values of VPM predicted GPP ", "(g " * C * " m"^{-2} * " day"^{-1} * ") averaged over 8 days")),
       color = expression("Topt (°C)")) +
  theme_minimal() +
  theme(text = element_text(size = 20), legend.position = "none")
...
ggsave(file.path(directory, "combined_plot.png"), plot = combined_plot, width = 14, height = 12)


# Save the plot
ggsave(file.path(directory, "Supplementaryfigure1.png"), combined_plot, width = 25, height = 14)# Define the weights for each metric
weight_rmse <- 0.33
weight_mae <- 0.33
weight_bias <- 0.33
...
sitesatellitemergedDATAsiteyear<-sitesatellitemergedDATA%>% dplyr::select(gpp, GPPvpmsite, GPP_site, siteyear.x, year, site, GPP_0.05_30.00, GPP_0_06512_31_79)
...
annualgppvpmcom_0.0530 <- ggplot(data = sitesatellitemergedDATAannualsum, aes(x = GPP_site, y = GPP_0.05_30.00, fill = site, shape = factor(year))) +
  ...
  geom_point(size = 22, stroke = 5, color = "black") +  # Darker outline for shapes
  geom_smooth(aes(group =1), se = FALSE, size = 8, method = lm)+
  xlab(bquote('Seasonal sum of GPP EC ('*g~ 'C'~ m^-2~season^-1*')')) +
  scale_x_continuous(breaks = seq(120, 250, by = 20))+
  ylab(expression(paste("Seasonal sum of ", GPP~VPM[site_biome],~'('*g~ 'C'~ m^-2~season^-1*')'))) +
  ...
  guides(fill = guide_legend(override.aes = list(shape = 21, stroke = 0.5, color = "black"))) +  # Darker outline in legend
  geom_text(aes(x = 144.7, y = 225, label = paste("RMSE =", round(GPPsiteVPMrmseannual_0.0530, 2))), size = 17, color = "black")+
  ggplot2::annotate("text", x = 159, y = 225, label = expression(paste("" * g ~ "C" ~ m^-2 ~ season^-1 * "")), size = 17, color = "black")+
  geom_text(aes(x = 144, y = 215, label = paste("MAE =", round(GPPsiteVPMmaeannual_0.0530, 2))), size = 17, color = "black")+
  ggplot2::annotate("text", x = 159, y = 215, label = expression(paste("" * g ~ "C" ~ m^-2 ~ season^-1 * "")), size = 17, color = "black")+
  geom_text(aes(x = 144, y = 205, label = paste("Bias =", round(GPPsiteVPMbiasannual_0.0530, 2))), size = 17, color = "black")+
  ggplot2::annotate("text", x = 159, y = 205, label = expression(paste("" * g ~ "C" ~ m^-2 ~ season^-1 * "")), size = 17, color = "black")+
  ggplot2::annotate("text", x = 140.6, y = 195, label = expression(paste("" * italic(R^2) * "=" * "")), size = 17, color = "black")+
  geom_text(aes(x = 145, y = 195, label = paste(round(GPPsiteVPMr2annual_0.0530, 2))), size = 17, color = "black")
...
annualgppvpmcom <- ggplot(data = sitesatellitemergedDATAannualsum, aes(x = GPP_site, y = GPP_0_06512_31_79, fill = site, shape = factor(year))) +
  ...
  xlab(bquote('Seasonal sum of GPP EC ('*g~ 'C'~ m^-2~season^-1*')')) +
  scale_x_continuous(breaks = seq(120, 250, by = 20))+
  ylab(expression(paste("Seasonal sum of ", GPP~VPM[site],~'('*g~ 'C'~ m^-2~season^-1*')'))) +
  scale_shape_manual(values = c(22, 21, 22, 25))+
  scale_fill_manual(values = pal) +
  scale_color_manual(values = pal) +
  labs(fill = "Site", shape = "Year") +  # Set legend names
  ...
  theme(plot.margin = grid::unit(c(0, 0, 0, 0), "mm")) +
  guides(fill = guide_legend(override.aes = list(shape = 21, stroke = 0.5, color = "black"))) +  # Darker outline in legend
  geom_text(aes(x = 144.7, y = 265, label = paste("RMSE =", round(GPPsiteVPMrmseannual, 2))), size = 17, color = "black")+
  ggplot2::annotate("text", x = 159, y = 265, label = expression(paste("" * g ~ "C" ~ m^-2 ~ season^-1 * "")), size = 17, color = "black")+
  geom_text(aes(x = 144, y = 255, label = paste("MAE =", round(GPPsiteVPMmaeannual, 2))), size = 17, color = "black")+
  ggplot2::annotate("text", x = 159, y = 255, label = expression(paste("" * g ~ "C" ~ m^-2 ~ season^-1 * "")), size = 17, color = "black")+
  geom_text(aes(x = 143.5, y = 245, label = paste("Bias =", round(GPPsiteVPMbiasannual, 2))), size = 17, color = "black")+
  ggplot2::annotate("text", x = 159, y = 245, label = expression(paste("" * g ~ "C" ~ m^-2 ~ season^-1 * "")), size = 17, color = "black")+
  ggplot2::annotate("text", x = 140.7, y = 235, label = expression(paste("" * italic(R^2) * "=" * "")), size = 17, color = "black")+
  geom_text(aes(x = 145, y = 235, label = paste(round(GPPsiteVPMr2annual, 2))), size = 17, color = "black")
...
# Filter the dataframe to include only the rows with the specified siteyears
validationdf <- sitesatellitemergedDATA %>%
  dplyr::filter(siteyear.y %in% validation_sites)
validationsetrmse<-Metrics::rmse(validationdf$GPP_site, validationdf$GPP_0_06512_31_79)
validationsetmae<-Metrics::mae(validationdf$GPP_site, validationdf$GPP_0_06512_31_79)
validationsetbias<-Metrics::bias(validationdf$GPP_site, validationdf$GPP_0_06512_31_79)
validationsetr2<-cor(validationdf$GPP_site,validationdf$GPP_0_06512_31_79)^2
plot(validationdf$GPP_site, validationdf$GPP_0_06512_31_79)
# Print the results in the desired format
result_text <- sprintf(
  "In a single run, the model produced GPP prediction of RMSE, MAE, bias and R2 of %.2f g C m-2 day-1, %.2f g C m-2 day-1, %.2f g C m-2 day-1 and %.2f.",
  validationsetrmse, validationsetmae, validationsetbias, validationsetr2
)
